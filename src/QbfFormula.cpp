...
qbfsort::QbfFormula qbfsort::QbfFormula::fromStream(std::istream &from) {
  QbfFormula formula;
...
void qbfsort::QbfFormula::toStream(const QbfFormula &formula,
                                   std::ostream &to) {
...
void qbfsort::QbfFormula::printStatistics(std::ostream &to) const {
  Json::Value root;
...
void qbfsort::QbfFormula::sortLiterals(
...
void qbfsort::QbfFormula::sortClauses(
    std::function<bool(std::vector<std::int32_t>, std::vector<std::int32_t>)>
        sorter) {
  std::sort(matrix.begin(), matrix.end(), sorter);
}

void qbfsort::QbfFormula::sortQuantifiers(
    std::function<bool(std::int32_t, std::int32_t)> sorter) {
  for (auto &p : prefix) {
...
std::int32_t
qbfsort::QbfFormula::getFrequencyLiteral(std::int32_t literal) const {
  return literal > 0 ? frequenciesPositiveLiterals.at(literal)
                     : frequenciesNegativeLiterals.at(-literal);
}

std::int32_t
qbfsort::QbfFormula::getFrequencyVariable(std::int32_t variable) const {
  return frequenciesPositiveLiterals.at(variable) +
         frequenciesNegativeLiterals.at(variable);
}

std::int32_t qbfsort::QbfFormula::getFrequencyClauseLiteralSum(
...
std::int32_t qbfsort::QbfFormula::getFrequencyClauseVariableSum(
    const std::vector<std::int32_t> &clause) const {
...
std::int32_t
qbfsort::QbfFormula::getCountedBinariesLiteral(std::int32_t literal) const {
  return getCountedBinariesByAssignment(literal);
}

std::int32_t
qbfsort::QbfFormula::getCountedBinariesVariable(std::int32_t variable) const {
  return getCountedBinariesByAssignment(std::abs(variable)) +
         getCountedBinariesByAssignment(-std::abs(variable));
}

std::int32_t qbfsort::QbfFormula::getCountedBinariesClauseLiteralSum(
    const std::vector<std::int32_t> &clause) const {
  std::int32_t literalSum{0};
  for (std::int32_t l : clause) {
...
std::int32_t qbfsort::QbfFormula::getCountedBinariesClauseVariableSum(
    const std::vector<std::int32_t> &clause) const {
  std::int32_t variableSum{0};
  for (std::int32_t l : clause) {
...
qbfsort::QbfFormula::getNewBinaryClausesByAssignment(
  ...
  for (const std::vector<std::int32_t> &v : matrix) {
    if (v.size() == 3 && std::find(v.begin(), v.end(), -literal) != v.end()) {
      newBinaryClauses.emplace(std::make_pair(v[0] != -literal ? v[0] : v[1],
                                              v[2] != -literal ? v[2] : v[1]));
    }
  }
...
void qbfsort::QbfFormula::precomputeFrequencies() {
...
float qbfsort::QbfFormula::getWeightedBinariesWeight(
    std::int32_t variable) const {
...
const qbfsort::QbfFormula::Quantifier qbfsort::QbfFormula::Quantifier::exists{
    QbfFormula::Quantifier("e")};

const qbfsort::QbfFormula::Quantifier qbfsort::QbfFormula::Quantifier::forall{
    QbfFormula::Quantifier("a")};

const qbfsort::QbfFormula::Quantifier &
qbfsort::QbfFormula::Quantifier::from_string(const std::string &token) {
...
const std::string &
qbfsort::QbfFormula::Quantifier::to_string(const Quantifier &quantifier) {
  return quantifier.token;
}

bool qbfsort::QbfFormula::Quantifier::isQuantifier(const std::string &token) {
  return token == exists.token || token == forall.token;
}

qbfsort::QbfFormula::Quantifier::Quantifier(const std::string &token)
    : token{token} {}

bool qbfsort::QbfFormula::Quantifier::operator==(
    const Quantifier &other) const {
  return this->token == other.token;
}

float qbfsort::QbfFormula::computeWeightedBinariesWeight(
    std::int32_t variable) const {
  return getWeightedBinariesHeuristic(variable) *
         getWeightedBinariesHeuristic(-variable);
}

float qbfsort::QbfFormula::getWeightedBinariesHeuristic(
    std::int32_t literal) const {
  float weight{0};
  std::set<std::pair<std::int32_t, std::int32_t>> newBinaryClauses{
...
float qbfsort::QbfFormula::getWeightedBinariesLiteralWeight(
    std::int32_t literal) const {
  ...
  std::size_t pos{static_cast<std::size_t>(
      literal > 0 ? literal : numberOfAtoms - literal)};
  if (weightedBinariesLiteralWeights[pos] < 0) {
    /* compute for the first time */
    weightedBinariesWeights[pos] =
        computeWeightedBinariesLiteralWeight(literal);
  }
  return weightedBinariesLiteralWeights[pos];
}

float qbfsort::QbfFormula::computeWeightedBinariesLiteralWeight(
    std::int32_t literal) const {
  float weight{0};
...
std::int32_t qbfsort::QbfFormula::getCountedBinariesByAssignment(
    std::int32_t literal) const {
  std::int32_t countedBinaries{0};
  for (const std::vector<std::int32_t> &v : matrix) {
    if (v.size() == 3 && std::find(v.begin(), v.end(), -literal) != v.end()) {
      ++countedBinaries;
    }
  }
  return countedBinaries;
}
